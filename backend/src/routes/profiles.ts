...
function getTeamSummaries(
  teams: Array<{ generation: number; gameId: number; updatedAt: Date; name: string }>
): TeamSummary[] {
  ...
  for (const team of teams) {
    ...
    if (team.updatedAt.toISOString() > existing.lastUpdatedAt) {
      existing.lastUpdatedAt = team.updatedAt.toISOString();
...
profiles.put("/me", authMiddleware, async (c) => {
  ...
  if (body.bio !== undefined) {
    ...
    const normalizedBio = body.bio.trim();
  ...
  const parsedGames = parseFavoriteGameIds(body.favoriteGameIds);
  if (parsedGames.error) return c.json({ error: parsedGames.error }, 400);

  const parsedPokemon = parseFavoritePokemonNames(body.favoritePokemonNames);

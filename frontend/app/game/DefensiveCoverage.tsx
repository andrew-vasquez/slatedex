...
const DefensiveCoverage = ({ coverage, generation }: DefensiveCoverageProps) => {
  ...
  const activeData = typeSummaries.find((item) => item.type === displayType) || typeSummaries[0];
    ...
    <div className="animate-section-reveal panel p-4 sm:p-5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="lg:h-full">
          <div className="coverage-heatmap-grid grid grid-cols-5 gap-1.5 sm:grid-cols-9 sm:gap-2" role="grid" aria-label="Type coverage heatmap" onPointerLeave={clearPreview}>
          ...
          <div className="coverage-detail-body">
            <div className="mb-3 grid grid-cols-3 gap-1.5">
              ...
              <div className="rounded-md px-2 py-1.5" style={{ background: "var(--surface-1)", border: "1px solid var(--border)" }}>
                ...
                <p className="font-mono text-xs" style={{ color: activeData.isLocked ? "var(--text-muted)" : getNetColor(activeData.net) }}>
                  {activeData.isLocked ? "—" : formatNet(activeData.net)}
                </p>
              </div>
            </div>
            ...
            <div className="coverage-detail-state">
              <p className="text-[0.72rem]" style={{ color: "var(--text-muted)" }}>
                {activeData.isLocked
                  ...
                  : hasCoverageData
                    ? "Review which teammates are weak or resistant to this type."

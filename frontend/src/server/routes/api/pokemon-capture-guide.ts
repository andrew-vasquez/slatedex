...
async function fetchEncounterEntriesForPokemon(
  ...
  const response = await fetch(locationUrl, { cache: "force-cache" });
  if (!response.ok) {
...
async function resolveCaptureGuide(pokemonId: number, versionId: string): Promise<CaptureGuideResponse> {
  ...
  if (targetSpecies.evolution_chain?.url) {
    try {
      const chainResponse = await fetch(targetSpecies.evolution_chain.url, { cache: "force-cache" });
      if (chainResponse.ok) {
...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(pokemonId) || pokemonId <= 0) {
    return Response.json({ error: "pokemonId must be a positive integer." }, { status: 400 });
  }
  ...
  try {
    ...
    return Response.json({ error: "Failed to load capture guide." }, { status: 500 });
  } finally {
    if (!inFlight) {

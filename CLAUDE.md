...
- **`/game/[gameId]`** is an RSC page that fetches Pokemon data server-side via `lib/pokeapi.ts`, then passes `PokemonPools` as props to the client `TeamBuilder`. Pages are statically generated via `generateStaticParams`.

`lib/pokeapi.ts` fetches from PokeAPI using `pokedex-promise-v2` at build time. It batches requests (50 per `Promise.all`), resolves evolution chains, type history, and regional dex membership. Results are cached in in-memory Maps.
...
`lib/constants.ts` has `TYPE_EFFECTIVENESS` and `TYPE_RESISTANCES` for all 18 types. `lib/teamAnalysis.ts` computes a `CoverageMap` (per-type weakness/resistance counts). This powers both the DefensiveCoverage heatmap and Smart Picks scoring.

### Version Exclusivity

`lib/versionExclusives.ts` has curated exclusive Pokemon lists per game (Gens 1-5), falling back to PokeAPI `game_indices`. Pokemon get an `exclusiveStatus` of "exclusive", "shared", or "unknown".

## Conventions

- `"use client"` on all interactive components; RSC only for pages/layout.
- Shared types in `lib/types.ts` (`Pokemon`, `Game`, `PokemonPools`, `CoverageMap`).
- `@/*` path alias maps to project root.
- `next/image` for sprites; remote patterns for `raw.githubusercontent.com`.
- Design tokens (colors, surfaces, glass effects) as CSS variables in `app/globals.css`. Dark theme default; light via `[data-theme="light"]`.
